...
def get_entities_and_relationships(sentence, model):
    ...
    prompt = (f"What are the entities and relationships for a knowledge graph in the following sentence: '{sentence}'? "
    ...
    for relationship in relations:
        ...
        if len(parts) == 2:
            ...
            knowledge_graph.add_edge(parts[0], parts[1], relation=f"{parts[0]} -> {parts[1]}")
        elif len(parts) == 3:
...
def task9(root_dir: str, model):
    ...
    for sentence in random_sentences:
        ...
        nx.draw_networkx_edge_labels(knowledge_graph, pos, edge_labels=nx.get_edge_attributes(knowledge_graph, "relation"))
